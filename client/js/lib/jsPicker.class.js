...
----------------------------------------------------------------------*/
var __calendarAutoFocusElement = null; //Текущий элемент для автофокуса
...
var Picker = new Class({
	...
	constructPicker: function(){
		...
		this.newContents = this.oldContents = new Element('div', {
			styles: {
				position: 'absolute',
				top: 0
			}
	...
	attach: function(attachTo, toggle){
		...
		var getOpenEvent = function(element){
			return function(event){
				...
				__calendarAutoFocusElement = element;
				setTimeout(function(element){if($(this) && __calendarAutoFocusElement == this){this.focus();this.select();}}.pass([], element), 100);
			};
		};
		...
		var getBlurEvent = function(element){
			return function(event){
				var input = element;
				...
				var cur_day, cur_month, cur_year;
				var re = /^((([1-9])|([0-2][0-9])|(30))[\.](((0[469])|([469])|(11)))|((([1-9])|([0-2][0-9])|(3[01]))[\.]((0[13578])|([13578])|(1[02])))|((([1-9])|([0-2][0-9]))[\.](2|02)))[\.]\d{4}$|^\d{4}$/;
				var i = value.indexOf(".");
				var j = value.indexOf(".",i + 1);
				var k = value.indexOf(".",j + 1);
				if (i != -1){
					cur_day = value.substr(0,i);
					if (j != -1) {
						cur_month = value.substr(i+1,j-i-1);
						cur_year = (k != -1) ? value.substr(j+1, k-j-1) : value.substr(j+1, value.length);
					}else{
						cur_month = value.substr(i+1,2);
						cur_year = value.substr(i+3,4);
					}
				}else{
					cur_day		= value.substr(0,2);
					cur_month	= value.substr(2,2);
					cur_year	= value.substr(4,4);
				}
				if(cur_day.length != 2){
					if(cur_day.length == 1) cur_day = '0'+cur_day;
					else cur_day = _NOW_DAY_T;
					cur_month = _NOW_MONTH_T; 
					cur_year = _NOW_YEAR;
				}
				...
				if(cur_year.length != 4){
					if(cur_year.length == 1)
						cur_year = '200'+cur_year;
					else
				...
				value = cur_day+'.'+cur_month+'.'+cur_year;
				if(!re.test(value)) value = _TODAY;
				input.value = value;
			}
		};
...
var DatePicker = Picker.Date = new Class({
	...
	* a string maxDate: new Date('3/4/2011'), // same
	...
	initialize : function(attachTo, options) {
		...
		//Задание минимальной и максимальной даты
		if (options.minDate) {
		...
		this.addEvent('attached', function(event, element) {
				...
				this.options.input_global.addEvent('keydown', function(event) { self.keydownEvent(event); });
		...
		var self = this;
		var picker = document.id(options.pickerClass + ' column_' + options.columns);
					/*
...
*/
	...
	constructPicker : function() {
		this.parent();
		this.previous = new Element('div.previous[html=&#160;]').inject(this.header).setProperty('role', 'button').setProperty('aria-label','Предыдущий месяц');
		this.next = new Element('div.next[html=&#160;]').inject(this.header).setProperty('role', 'button').setProperty('aria-label','Следующий месяц');
	},
	...
	renderYears : function(date, fx) {
		...
		for ( var i = pages; i--;) {
			...
			_columns.push(renderers.years(timesSelectors.years(
							options, _date.clone()), options, this.date.clone(), this.dateElements, function(date) {
		...
		var limitRight = (options.maxDate && (date.get('year') + options.yearsPerPage) >= options.maxDate.get('year'));
	...
	renderMonths : function(date, fx) {
		...
		var options = this.options, years = options.columns, _columns = [], _dates = [], iterateDate = date.clone().decrement('year', Math.floor((years - 1) / 2));
		this.dateElements = [];
		...
		// Set limits
		var year = date.get('year'), limitLeft = (options.minDate && year <= options.minDate.get('year'));
		var limitRight = (options.maxDate && year >= options.maxDate.get('year'));
		this[(limitLeft ? 'hide' : 'show') + 'Previous']();
		this[(limitRight ? 'hide' : 'show') + 'Next']();
		...
		var titleEvent = (canGoUp) ? function() {this.renderYears(date, 'fade');}.bind(this) : null;
	...
	renderDays : function(date, fx) {
		...
		var _columns = [];
		var _dates = [];
		var iterateDate = date.clone().decrement('month', Math.floor((months - 1) / 2));
		this.dateElements = [];
		...
		var limitLeft = (options.minDate && yearmonth <= options.minDate.format('%Y%m'));
		var limitRight = (options.maxDate && yearmonth >= options.maxDate.format('%Y%m'));
		this[(limitLeft ? 'hide' : 'show') + 'Previous']();
		this[(limitRight ? 'hide' : 'show') + 'Next']();

		this.setPreviousEvent(function() {
			this.renderDays(date.decrement('month', months), 'fade');
		}.bind(this));

		this.setNextEvent(function() {
			this.renderDays(date.increment('month', months), 'fade');
		}.bind(this));

		var canGoUp = options.pickOnly != 'days' || options.canAlwaysGoUp.contains('days');
		var titleEvent = (canGoUp) ? function() {this.renderMonths(date, 'fade');}.bind(this) : null;
		this.setTitleEvent(titleEvent);
	...
	renderTime : function(date, fx) {
		...
		if (originalColumns != 1) this.setColumns(1); // recursion

		this.setContent(renderers.time(options, date.clone(), function(date) {this.select(date);}.bind(this)), fx);

		// Hide buttons
		this.hidePrevious().hideNext().setPreviousEvent(null).setNextEvent(null);

		var canGoUp = options.pickOnly != 'time' || options.canAlwaysGoUp.contains('time');
		var titleEvent = (canGoUp) ? function() {this.setColumns(originalColumns, 'days', date, 'fade');}.bind(this) : null;
		this.setTitleEvent(titleEvent);
	...
	select : function(date, all) {
		...
		this.date = date;
		var formatted = date.format(this.options.format), time = date.strftime();
